...
export default function TypingInterface({ lessonId, onComplete, onBack, startInQuizMode = false }: TypingInterfaceProps) {
  ...
  const renderText = () => {
    // Flashcard mode: Show exactly 3 characters at a time
    const chunkSize = 3;
    const currentChunk = Math.floor(currentIndex / chunkSize);
    const startIndex = currentChunk * chunkSize;
    const endIndex = Math.min(currentText.length, startIndex + chunkSize);

    const visibleText = currentText.slice(startIndex, endIndex);
    ...
    return (
      <div className="text-center">
        ...
        <div className="mt-4 text-sm text-gray-500">
          Chunk {currentChunk + 1} of {Math.ceil(currentText.length / chunkSize)}
        </div>
      </div>
    );
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-gray-600 mt-1">
              {isQuizMode ? '⏱️ Timed Quiz' : '🏃 Practice Mode (Untimed)'}
            </p>
          </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          <p className="text-blue-800">
            <strong>Instructions:</strong> Type the text exactly as shown above.
            The current character is highlighted in blue.
